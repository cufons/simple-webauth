...
class AuthOnly():
    ...
    def __call__(self,request):
        authid = binascii.a2b_base64(request.COOKIES.get('chi_auth_id'))
        authkey = binascii.a2b_base64(request.COOKIES.get('chi_auth_key'))
...
def login(request):
    if request.method == 'POST':
        ...
        r.set_cookie('chi_auth_id',binascii.b2a_base64(tokens['id'],newline=False).decode(),httponly=True,secure=True)
        r.set_cookie('chi_auth_key',binascii.b2a_base64(tokens['key'],newline=False).decode(),httponly=True,secure=True)
        return r
    else:
...
def loginview(request,failed=False):
    context = {'error' : failed,'redir_success' : request.GET.get('redirect') or '/'}
